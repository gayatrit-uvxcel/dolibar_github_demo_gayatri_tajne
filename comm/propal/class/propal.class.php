...
class Propal extends CommonObject
{
     ...
     *        @see           add_product()
     */
    public function addline($desc, $pu_ht, $qty, $txtva, $txlocaltax1 = 0.0, $txlocaltax2 = 0.0, $fk_product = 0, $remise_percent = 0.0, $price_base_type = 'HT', $pu_ttc = 0.0, $info_bits = 0, $type = 0, $rang = -1, $special_code = 0, $fk_parent_line = 0, $fk_fournprice = 0, $pa_ht = 0, $label = '', $date_start = '', $date_end = '', $array_options = 0, $fk_unit = null, $unit = null, $category = null,$fk_projectid = null, $fk_socid = null, $origin = '', $origin_id = 0, $pu_ht_devise = 0, $fk_remise_except = 0, $noupdateafterinsertline = 0 )
    {

        global $mysoc, $conf, $langs;

        dol_syslog(get_class($this) . "::addline propalid=$this->id, desc=$desc, pu_ht=$pu_ht, qty=$qty, txtva=$txtva, fk_product=$fk_product, remise_except=$remise_percent, price_base_type=$price_base_type, pu_ttc=$pu_ttc, info_bits=$info_bits, type=$type, category=$category,fk_remise_except=" . $fk_remise_except);
        ...
        if ($this->statut == self::STATUS_DRAFT) {
            ...
            $this->line->fk_socid = $fk_socid;
            $this->line->fk_projectid = $fk_projectid;
    ...
    public function create($user, $notrigger = 0)
    {
        ...
        $sql .= ", " . ($this->contact ? "'" . $this->db->escape($this->contact) . "'" : "null");
        $sql .= ", " . ($this->cell ? "'" . $this->db->escape($this->cell) . "'" : "null");
        $sql .= ", " . ($this->email ? "'" . $this->db->escape($this->email) . "'" : "null");
        $sql .= ", " . ($this->client_vat ? "'" . $this->db->escape($this->client_vat) . "'" : "null");
        $sql .= ", " . ($this->division ? "'" . $this->db->escape($this->division) . "'" : "null");
    ...
    public function fetch($rowid, $ref = '', $ref_ext = '', $forceentity = 0)
    {
        ...
        $sql .= ", p.contact_person,p.email,p.client_vat,p.division,p.vendor_no,p.po_no,p.cell,p.vat_no,p.quote_no,p.terms_and_conditions,p.notes";
        ...
        if ($resql) {
            if ($this->db->num_rows($resql)) {
                ...
                $this->contact = $obj->contact_person;
                $this->email = $obj->email;
                $this->client_vat = $obj->client_vat;
                $this->division = $obj->division;
                $this->vendor_no = $obj->vendor_no;
                $this->po_no = $obj->po_no;
                $this->cell = $obj->cell;
                $this->vat_no = $obj->vat_no;
                $this->quote_no = $obj->quote_no;
                $this->terms_and_conditions = $obj->terms_and_conditions;
    ...
    public function fetch_lines($only_product = 0, $loadalsotranslation = 0, $filters = '')
    {
        ...
        $sql .= ' d.fk_unit,';
        $sql .= ' d.unit,d.category,d.fk_socid,d.fk_projectid,';
        ...
        if ($result) {
            ...
            while ($i < $num) {
                ...
                $line->fk_socid = $objp->fk_socid;
                $line->fk_projectid = $objp->fk_projectid;
...
class PropaleLigne extends CommonObjectLine
{
    ...
    public function insert($notrigger = 0)
    {
        ...
        $sqlPropal .= " " . ($this->category ? "'" . $this->db->escape($this->category) . "'" : "null") . "";
        ...
        $sqlPropal .= " " . price2num($this->tva_tx) . ",";
        $sqlPropal .= " " . price2num($this->localtax1_tx) . ",";
        $sqlPropal .= " " . price2num($this->localtax2_tx) . ",";
        $sqlPropal .= " '" . $this->db->escape($this->localtax1_type) . "',";
        $sqlPropal .= " '" . $this->db->escape($this->localtax2_type) . "',";
        $sqlPropal .= " " . (price2num($this->subprice) !== '' ? price2num($this->subprice, 'MU') : "null") . ",";
        $sqlPropal .= " " . price2num($this->remise_percent, 3) . ",";
        $sqlPropal .= " " . (isset($this->info_bits) ? ((int) $this->info_bits) : "null") . ",";
        $sqlPropal .= " " . price2num($this->total_ht, 'MT') . ",";
        $sqlPropal .= " " . price2num($this->total_tva, 'MT') . ",";
        $sqlPropal .= " " . price2num($this->total_localtax1, 'MT') . ",";
        $sqlPropal .= " " . price2num($this->total_localtax2, 'MT') . ",";
        $sqlPropal .= " " . price2num($this->total_ttc, 'MT') . ",";
        $sqlPropal .= " " . (!empty($this->fk_fournprice) ? "'" . $this->db->escape($this->fk_fournprice) . "'" : "null") . ",";
        $sqlPropal .= " " . (isset($this->pa_ht) ? "'" . price2num($this->pa_ht) . "'" : "null") . ",";
        $sqlPropal .= ' ' . ((int) $this->special_code) . ',';
        $sqlPropal .= ' ' . ((int) $this->rang) . ',';
        $sqlPropal .= ' ' . (empty($this->fk_unit) ? 'NULL' : ((int) $this->fk_unit)) . ',';
        $sqlPropal .= " " . (!empty($this->date_start) ? "'" . $this->db->idate($this->date_start) . "'" : "null") . ',';
        $sqlPropal .= " " . (!empty($this->date_end) ? "'" . $this->db->idate($this->date_end) . "'" : "null");
        $sqlPropal .= ", " . ($this->fk_multicurrency > 0 ? ((int) $this->fk_multicurrency) : 'null');
        $sqlPropal .= ", '" . $this->db->escape($this->multicurrency_code) . "'";
        $sqlPropal .= ", " . price2num($this->multicurrency_subprice, 'CU');
        ...
        $sqlPropal .= ", " . ($this->fk_projectid ? "'" . $this->db->escape($this->fk_projectid) . "'" : "null") . "";
        $sqlPropal .= ", " . ($this->fk_socid ? "'" . $this->db->escape($this->fk_socid) . "'" : "null") . "";
        $sqlPropal .= ", " . ($this->unit ? "'" . $this->db->escape($this->unit) . "'" : "null") . ")";
        // Insert line into database
        $sqlFacture = 'INSERT INTO ' . MAIN_DB_PREFIX . 'facturedet';
        $sqlFacture .= ' (fk_facture, fk_parent_line, label, description, fk_product, product_type, category,';
        $sqlFacture .= ' fk_remise_except, qty, vat_src_code, tva_tx, localtax1_tx, localtax2_tx, localtax1_type, localtax2_type,';
        $sqlFacture .= ' subprice, remise_percent, ';
        $sqlFacture .= ' info_bits, ';
        $sqlFacture .= ' total_ht, total_tva, total_localtax1, total_localtax2, total_ttc, fk_product_fournisseur_price, buy_price_ht, special_code, rang,';
        $sqlFacture .= ' fk_unit,';
        $sqlFacture .= ' date_start, date_end';
        $sqlFacture .= ', fk_multicurrency, multicurrency_code, multicurrency_subprice, multicurrency_total_ht, multicurrency_total_tva, multicurrency_total_ttc,fk_projectid,fk_socid,unit)';
        $sqlFacture .= " VALUES (" . $this->fk_propal . ",";
        $sqlFacture .= " " . ($this->fk_parent_line > 0 ? "'" . $this->db->escape($this->fk_parent_line) . "'" : "null") . ",";
        $sqlFacture .= " " . (!empty($this->label) ? "'" . $this->db->escape($this->label) . "'" : "null") . ",";
        ...
        $sqlFacture .= " " . ($this->category ? "'" . $this->db->escape($this->category) . "'" : "null") . "";
        ...
        $sqlFacture .= " " . price2num($this->tva_tx) . ",";
        $sqlFacture .= " " . price2num($this->localtax1_tx) . ",";
        $sqlFacture .= " " . price2num($this->localtax2_tx) . ",";
        $sqlFacture .= " '" . $this->db->escape($this->localtax1_type) . "',";
        $sqlFacture .= " '" . $this->db->escape($this->localtax2_type) . "',";
        $sqlFacture .= " " . (price2num($this->subprice) !== '' ? price2num($this->subprice, 'MU') : "null") . ",";
        $sqlFacture .= " " . price2num($this->remise_percent, 3) . ",";
        $sqlFacture .= " " . (isset($this->info_bits) ? ((int) $this->info_bits) : "null") . ",";
        $sqlFacture .= " " . price2num($this->total_ht, 'MT') . ",";
        $sqlFacture .= " " . price2num($this->total_tva, 'MT') . ",";
        $sqlFacture .= " " . price2num($this->total_localtax1, 'MT') . ",";
        $sqlFacture .= " " . price2num($this->total_localtax2, 'MT') . ",";
        $sqlFacture .= " " . price2num($this->total_ttc, 'MT') . ",";
        $sqlFacture .= " " . (!empty($this->fk_fournprice) ? "'" . $this->db->escape($this->fk_fournprice) . "'" : "null") . ",";
        $sqlFacture .= " " . (isset($this->pa_ht) ? "'" . price2num($this->pa_ht) . "'" : "null") . ",";
        $sqlFacture .= ' ' . ((int) $this->special_code) . ',';
        $sqlFacture .= ' ' . ((int) $this->rang) . ',';
        $sqlFacture .= ' ' . (empty($this->fk_unit) ? 'NULL' : ((int) $this->fk_unit)) . ',';
        $sqlFacture .= " " . (!empty($this->date_start) ? "'" . $this->db->idate($this->date_start) . "'" : "null") . ',';
        $sqlFacture .= " " . (!empty($this->date_end) ? "'" . $this->db->idate($this->date_end) . "'" : "null");
        $sqlFacture .= ", " . ($this->fk_multicurrency > 0 ? ((int) $this->fk_multicurrency) : 'null');
        $sqlFacture .= ", '" . $this->db->escape($this->multicurrency_code) . "'";
        $sqlFacture .= ", " . price2num($this->multicurrency_subprice, 'CU');
        ...
        $sqlFacture .= ", " . ($this->fk_projectid ? "'" . $this->db->escape($this->fk_projectid) . "'" : "null") . "";
        $sqlFacture .= ", " . ($this->fk_socid ? "'" . $this->db->escape($this->fk_socid) . "'" : "null") . "";
        $sqlFacture .= ", " . ($this->unit ? "'" . $this->db->escape($this->unit) . "'" : "null") . ")";
