	...
	if (empty($noout)) {
		...
		for ($i = 0; $i < $num; $i++) {     // Loop on each menu entry
			...
			if ($menu_array[$i]['level'] > 0) {
				...
				if (!in_array($titre, $excluded_values)) {
					$cssmenu = '';
					if ($menu_array[$i]['url']) {
...
function get_left_menu_commercial($mainmenu, &$newmenu, $usemenuhider = 1, $leftmenu = 'none', $type_user = 0)
{
	...
	if ($mainmenu == 'commercial') {
		...
		if (isModEnabled('propal')) {
			$langs->load("propal");
			$newmenu->add("/comm/propal/index.php?leftmenu=propals", $langs->trans("Quotations"), 0, $user->hasRight('propal',  'read'), '', $mainmenu, 'propals', 100, '', '', '', img_picto('', 'propal', 'class="paddingright pictofixedwidth"'));
			$newmenu->add("/comm/propal/card.php?action=create&amp;leftmenu=propals", $langs->trans("New Quotation"), 1, $user->hasRight('propal',  'write'));
...
function get_left_menu_members($mainmenu, &$newmenu, $usemenuhider = 1, $leftmenu = 'none', $type_user = 0)
{
	...
	if ($mainmenu == 'members') {
		if (isModEnabled('adherent')) {
			...
			$newmenu->add("/adherents/index.php?leftmenu=members&amp;mainmenu=members", $langs->trans("Member"), 0, $user->hasRight('adherent', 'read'), '', $mainmenu, 'members', 0, '', '', '', img_picto('', 'member', 'class="paddingright pictofixedwidth"'));
