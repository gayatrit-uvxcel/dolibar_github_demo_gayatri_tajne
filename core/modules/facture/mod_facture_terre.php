...
class mod_facture_terre extends ModeleNumRefFactures
{
    ...
    public function __construct()
    {
        ...
        if ((float) $conf->global->MAIN_VERSION_LAST_INSTALL >= 16.0 && $mysoc->country_code != 'FR') {
            $this->prefixinvoice = 'FVAI'; // We use correct standard code "IN = Invoice"
    ...
    public function getNextValue($objsoc, $invoice, $mode = 'next')
    {
		...
		print '<script>console.log(`Propal: '.$invoice->date.'`)</script>';
        ...
        if ($mode == 'last') {
            ...
            $yymmdd = strftime("%Y%m%d", $date); // Change to get year, month, and day in the desired format

            // Check if any invoice exists for the current date
            $sql = "SELECT COUNT(*) AS invoice_count FROM " . MAIN_DB_PREFIX . "facture";
            $sql .= " WHERE ref LIKE '" . $db->escape($prefix) . "-" . $yymmdd . "%'";
            $sql .= " AND entity IN (" . getEntity('invoicenumber', 1, $invoice) . ")";

            $resql = $db->query($sql);
            if ($resql) {
                $obj = $db->fetch_object($resql);
                if ($obj && $obj->invoice_count > 0) {
                    // If invoices exist for the current date, generate the number in format FVAI-20231214-XX
                    // Retrieve the maximum invoice number for the current date
                    $sql = "SELECT MAX(SUBSTRING_INDEX(ref, '-', -1)) AS max_num FROM " . MAIN_DB_PREFIX . "facture";
                    $sql .= " WHERE ref LIKE '" . $db->escape($prefix) . "-" . $yymmdd . "-%'";
                    $sql .= " AND entity IN (" . getEntity('invoicenumber', 1, $invoice) . ")";
                    ...
                    if ($resql) {
                        $obj = $db->fetch_object($resql);
                        if ($obj && $obj->max_num !== null) {
                            $max = intval($obj->max_num); // Retrieve the maximum number and convert to integer
                        } else {
                            $max = 0; // If no previous invoices found, start from 0
                        }
                        $num = sprintf("%02d", $max + 1); // Increment the number and format it with leading zeros

                        dol_syslog(get_class($this) . "::getNextValue return " . $prefix . "-" . $yymmdd . "-" . $num);
                        return $prefix . "-" . $yymmdd . "-" . $num;
                    } else {
                        dol_print_error($db);
                    }
                } else {
                    // If no invoices exist for the current date, generate the number in format FVAI-20231214
                    dol_syslog(get_class($this) . "::getNextValue return " . $prefix . "-" . $yymmdd);
