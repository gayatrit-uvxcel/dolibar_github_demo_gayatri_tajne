...
if ($object->element == 'supplier_proposal' || $object->element == 'order_supplier' || $object->element == 'invoice_supplier' || $object->element == 'invoice_supplier_rec') { // We must have same test in printObjectLines
    ...
    ?>
		...
		<td class="linecoluht right"><span id="title_up_ht"><?php echo $langs->trans('Per'); ?></span></td>
		...
		<?php if (!empty($inputalsopricewithtax) && !getDolGlobalInt('MAIN_NO_INPUT_PRICE_WITH_TAX')) {?>
			<td class="linecoluttc right"><span id="title_up_ttc"><?php echo $langs->trans('Per(inc. tax)'); ?></span></td>
		<?php }?>
		<td class="linecolqty right"><?php echo $langs->trans('Qty'); ?></td>
		<td class="linecoluht center"><?php echo $langs->trans('Unit'); ?></td>
		<?php
...
?> // Deal with supplier ref price (idprodfournprice=int) if (jQuery('#idprodfournprice').val()> 0)
					{
...
if (!empty($conf->global->FCKEDITOR_ENABLE_DETAILS)) {
        ...
        ?> jQuery('#dp_desc').text(description); <?php
}
}
