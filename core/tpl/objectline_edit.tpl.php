...
$sql_llx_propaldet = "SELECT category FROM " . MAIN_DB_PREFIX . "propaldet WHERE rowid = $line->rowid";
...
if (!$situationinvoicelinewithparent) {
    print '<td><input type="text" name="category" value="' . $line->category . '" readonly /></td>';
}
...
$coldisplay++;
if (($line->unit)) {
    print '<input type="text" class="flat right width40" name="unit" id="unit" value="' . (GETPOSTISSET('unit') ? GETPOST('unit') : $line->unit) . '"';
    if ($situationinvoicelinewithparent) {
        print ' readonly';
    }
} else {?>
