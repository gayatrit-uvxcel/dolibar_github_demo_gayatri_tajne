...
class Facture extends CommonInvoice
{
    ...
    public function create(User $user, $notrigger = 0, $forceduedate = 0)
    {
        ...
        if ($resql) {
            ...
            $invoice_no = '(PROV' . $this->id . ')';
            $sql = 'UPDATE ' . MAIN_DB_PREFIX . "facture SET invoice_no='" . $this->db->escape($invoice_no) . "' WHERE rowid=" . ((int) $this->id);
            $resql = $this->db->query($sql);
            if (!$resql) {
                $error++;
            }
                ...
                foreach ($this->lines as $i => $val) {
                    ...
                    if ($result >= 0) {
                        ...
                        $result = $this->addline(
                            ...
                            1,
                            $newinvoiceline->unit,
                            $newinvoiceline->fk_projectid,
                            $newinvoiceline->fk_socid,
            ...
            if (!$error && $this->fac_rec > 0) {
                foreach ($_facrec->lines as $i => $val) {
                    ...
                    $result_insert = $this->addline(
                        ...
                        $_facrec->lines[$i]->fk_projectid,
                        $_facrec->lines[$i]->fk_socid,
    ...
    public function fetch($rowid, $ref = '', $ref_ext = '', $notused = '', $fetch_situation = false)
    {
        ...
        $sql .= ', f.email, f.client_vat,f.division,f.vendor_no,f.po_no,f.quote_no,f.invoice_no,f.vat_no,f.cell,f.contact_person,f.delivery_no';
        ...
        if ($resql) {
            if ($this->db->num_rows($resql)) {
                ...
                $this->delivery_no = $obj->delivery_no;
    ...
    public function validate($user, $force_number = '', $idwarehouse = 0, $notrigger = 0, $batch_rule = 0)
    {
        ...
        if ($num) {
            ...
            // updating ref after Validating invoice
            $sql = 'UPDATE ' . MAIN_DB_PREFIX . 'facture';
            $sql .= " SET ref = '" . $this->db->escape($num) . "',";
            $sql .= " invoice_no = '" . $this->db->escape($num) . "',";
            $sql .= " delivery_no = '" . str_replace('I', 'D', $this->db->escape($num)) . "',";
            $sql .= " fk_statut = " . self::STATUS_VALIDATED . ", fk_user_valid = " . ($user->id > 0 ? $user->id : "null") . ", date_valid = '" . $this->db->idate($now) . "'";
            if (!empty($conf->global->FAC_FORCE_DATE_VALIDATION)) { // If option enabled, we force invoice date
    ...
    public function addline(
        ...
        dol_syslog(get_class($this) . "::addline id=$this->id, pu_ht=$pu_ht, qty=$qty, txtva=$txtva, txlocaltax1=$txlocaltax1, txlocaltax2=$txlocaltax2, fk_product=$fk_product, remise_percent=$remise_percent, date_start=$date_start, date_end=$date_end, ventil=$ventil, info_bits=$info_bits, fk_remise_except=$fk_remise_except, price_base_type=$price_base_type, pu_ttc=$pu_ttc, type=$type, fk_unit=$fk_unit, unit=$unit, desc=" . dol_trunc($desc, 25), LOG_DEBUG);
     ...
     *  @return        int                             < 0 if KO, > 0 if OK
     */
    public function updateline($rowid, $desc, $pu, $qty, $remise_percent, $date_start, $date_end, $txtva, $txlocaltax1 = 0, $txlocaltax2 = 0, $price_base_type = 'HT', $info_bits = 0, $type = self::TYPE_STANDARD, $category = null, $fk_parent_line = 0, $skip_update_total = 0, $fk_fournprice = null, $pa_ht = 0, $label = '', $special_code = 0, $array_options = 0, $situation_percent = 100, $fk_unit = null, $unit = null, $pu_ht_devise = 0, $notrigger = 0, $ref_ext = '', $rang = 0)
    {
        ...
        dol_syslog(get_class($this) . "::updateline rowid=$rowid, desc=$desc, pu=$pu, qty=$qty, remise_percent=$remise_percent, date_start=$date_start, date_end=$date_end, txtva=$txtva, txlocaltax1=$txlocaltax1, txlocaltax2=$txlocaltax2, price_base_type=$price_base_type, info_bits=$info_bits, type=$type, fk_parent_line=$fk_parent_line pa_ht=$pa_ht, special_code=$special_code, fk_unit=$fk_unit, unit=$unit, pu_ht_devise=$pu_ht_devise", LOG_DEBUG);
...
class FactureLigne extends CommonInvoiceLine
{
    ...
    public function fetch($rowid)
    {
        ...
        $sql .= ' fd.fk_socid, fd.fk_projectid,';
        $sql .= ' fd.fk_unit, fd.unit, fd.fk_user_author, fd.fk_user_modif,';
        ...
        if ($result) {
            ...
            $this->unit = $objp->unit;
            $this->fk_socid = $objp->fk_socid;
            $this->fk_projectid = $objp->fk_projectid;
    ...
    public function insert($notrigger = 0, $noerrorifdiscountalreadylinked = 0)
    {
        ...
        $sql .= ' fk_multicurrency, multicurrency_code, multicurrency_subprice, multicurrency_total_ht, multicurrency_total_tva, multicurrency_total_ttc, fk_projectid, fk_socid, unit';
        $sql .= ')';
    ...
    public function update($user = '', $notrigger = 0)
    {
        ...
        $sql .= ", category='" . (empty($this->category) ? '' : $this->category) . "'";
        ...
        $sql .= ", fk_unit=" . (!$this->fk_unit ? 'NULL' : 'NULL');
