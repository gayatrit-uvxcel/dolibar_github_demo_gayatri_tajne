    ...
    if ($action == 'confirm_clone' && $confirm == 'yes' && $permissiontoadd) {
        ...
        if ((GETPOST('type') == Facture::TYPE_STANDARD || GETPOST('type') == Facture::TYPE_DEPOSIT || GETPOST('type') == Facture::TYPE_PROFORMA || (GETPOST('type') == Facture::TYPE_SITUATION && !GETPOST('situations'))) && GETPOST('fac_rec') <= 0) {
            ...
            if (!$error) {
                ...
                // Save the custom email field to the database
                $sql = "UPDATE llx_facture SET email = '" . $db->escape($email) . "' WHERE rowid = " . $object->socid;
                $resql = $db->query($sql);
        ...
        $unit = (GETPOST('unit') ? GETPOST('unit') : 0);
        $pu_ht_devise = price2num(GETPOST('multicurrency_subprice'), '', 2);
        $pu_ttc_devise = price2num(GETPOST('multicurrency_subprice_ttc'), '', 2);
        ...
        if (
            in_array($object->statut, array(Facture::STATUS_CLOSED, Facture::STATUS_VALIDATED))
            && $object->type == Facture::TYPE_SITUATION
            && $usercancreate
            && !$objectidnext
...
if ($action == 'create') {
    ...
    // print '<form name="add"  method="POST" id="test" onsubmit="event.preventDefault(); logFormData(event);">';
    ...
    // print '<tr><td class="fieldrequired">'.$langs->trans('Panel Manufacture').'</td>';
    // print '<td colspan="2">';
    // print '</td>';
    // print '</tr>'."\n";
    ...
    if ($socid > 0) {
        ...
        if (empty($conf->global->INVOICE_DISABLE_REPLACEMENT)) {
            ...
            print '<div class="tagtr listofinvoicetype" style="display:none;"><div class="tagtd listofinvoicetype">';
    ...
    // custome fields code written by gayatri
    ...
    if ($socid > 0) {
        ...
        if ($projectid > 0) {
            ...
            $sql_llx_propal = "SELECT * FROM " . MAIN_DB_PREFIX . "propal WHERE fk_projet = $projectid";
            ...
            print '<tr><td>' . $langs->trans('Client VAT:') . '</td><td colspan="2">';
            print '<input type="text" id="client_vat" name="client_vat" value="' . $object->client_vat . '" readonly />';
            print '</td></tr>';
            ...
            print '<input type="text" id="quote_no" name="quote_no" value="' . $object->quote_no . '" readonly />';
            print '</td></tr>';
    ...
    if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline') {
        ...
        if (empty($reshook)) {
            ...
            if ($object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_SITUATION) {
                ...
                // For credit note
                if (
                    $object->type == Facture::TYPE_CREDIT_NOTE && $object->statut == Facture::STATUS_VALIDATED && $object->paye == 0 && $usercancreate
            ...
            // Classify paid
            if (
                $object->statut == Facture::STATUS_VALIDATED && $object->paye == 0 && $usercanissuepayment && (
